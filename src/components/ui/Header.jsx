...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 md:left-[300px] right-0 h-[76px] z-30 ">
      {/* Desktop Header */}
      <div className="h-full  md:flex hidden items-center justify-between 2xl:gap-32 gap-4 pl-10">
        <nav className="flex items-center justify-between gap-2 md:gap-4 overflow-x-auto scrollbar-hide bg-white rounded-full w-[75%]">
          {navItems.map((item) => (
            <button
              key={item.name}
              onClick={() => setActiveTab(item.name)}
              className={`px-2 py-1 rounded-full 2xl:text-base text-[12px] text-nowrap font-medium flex items-center gap-1.5 transition-colors w-full ${
                activeTab === item.name
